...
int main(int argc, char *argv[]){
    ...
    int running = 1;
      while(running){
		...
		// forking, creating a child and parent process
		
		int forkId = fork(); // make a child process
		if(forkId == -1){ printf("Forking Error\n"); } 
		// if it fails, will use parent process instead
		if(forkId == 0){ printf("Current: Child process\n"); }
		else{
			wait(NULL); // parent waits until child is finished executing
        ...
        while(1){
          ...
          for(int i = 0; i < 100; i++){ incomingCase[i] = toupper(incoming[i]); } // ensures that it is case insensitive
			...
			if(strncmp(incomingCase, "BYE", 3) == 0){
				...
				if(forkId == 0){ exit(0); } // terminate the child process
				else{
					close(clientfd);
					running = 0;
				}
